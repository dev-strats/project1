...
import {fifthTextColor, tooltip_data, editStrategyColor} from '../../config';
...
const StrategyPage = ({
    ...
    collapseClick}) => ({
    render() {
        let CommonParametersHeight = $('#CommonParameters').length > 0 ? $('#CommonParameters')[0].clientHeight : heightBlock.ParametersStrategy;
        ...
        if(this.props.strategy.name){
            if(this.props.tabs_nav_content_has_change){
                if (this.props.tabs_nav_content_has_change[this.props.strategy.name]) {
                    styles_of_strategy = {
        ...
        let disable_button = this.props.strategy && this.props.time_series && this.props.tabs_nav_content_has_change[this.props.strategy.name] ? false : true;
        return (
            <div style={Object.assign(styles.containerBox, styles_of_strategy)}>
                ...
                <Row className="show-grid border-top-class">
                    <Col id="CommonParameters" xs={12} md={4} className="p-t-30 p-b-15 border-right-class" style={{minHeight: heightBlock.ParametersStrategy}} data-tip data-for='parameters_strategy'>
                ...
                <Row className="show-grid p-t-15 p-b-15 border-top-class" >
                    <Col xs={12} className="text-center">
                        <Button bsStyle="primary" id={this.props.strategy ? this.props.strategy.id : 0}
                            className="m-r-30" onClick={updateButton} 
